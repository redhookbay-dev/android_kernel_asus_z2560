...
struct delayed_work hp_amp_work, spk_amp_work;
...
//	{RT5640_I2S1_SDP	, 0xD000},//change IIS1 and IIS2
	/*record*/
	{RT5640_IN1_IN2		, 0x0000},//IN1 boost 0db and signal ended mode
...
//		mod_timer( &mclk_check_timer, jiffies);
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		cancel_delayed_work_sync(&spk_amp_work);
		snd_soc_update_bits(codec, RT5640_PWR_DIG1,
			RT5640_PWR_CLS_D, 0);
		snd_soc_update_bits(codec, RT5640_SPK_VOL,
			RT5640_L_MUTE | RT5640_R_MUTE,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (PROJ_ID == PROJ_ID_PF400CG) { 
			...
			if (AX_MicroP_getHWID() == 3) { // ER1-1 => differential
				...
				AX_MicroP_setGPIOOutputPin(OUT_up_SPK_SEL, 1);
				if (p72g_atd_device == 1) { // RCV test
					snd_soc_update_bits(codec, RT5640_OUTPUT, 0x0080, 0x0);
					p72g_atd_device = 0;
					pr_info("%s : p72g_atd_device 1 enable RCV, mute LOUTR\n", __func__);
				} else if (p72g_atd_device == 2) { // SPK test
					snd_soc_update_bits(codec, RT5640_OUTPUT, 0x8000, 0x0);
					p72g_atd_device = 0;
					pr_info("%s : p72g_atd_device 2 enable SPK, mute LOUTL\n", __func__);
				} else if (output_device_from_hal() == 0x2) { // LOUTR -> P72G Spk, mute LOUTL
					snd_soc_update_bits(codec, RT5640_OUTPUT, 0x8000, 0x0);
					pr_info("%s : enable SPK, mute LOUTL\n", __func__);
				} else if (output_device_from_hal() == 0x1) { // LOUTL -> P72G Rcv, mute LOUTR
					snd_soc_update_bits(codec, RT5640_OUTPUT, 0x0080, 0x0);
			     ...
			     struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		if (PROJ_ID == PROJ_ID_PF400CG || PROJ_ID == PROJ_ID_ME175CG) {
...
//		}
		if (SND_SOC_BIAS_OFF == codec->dapm.bias_level) {
			...
			snd_soc_update_bits(codec, RT5640_PWR_ANLG1,
				...
				RT5640_PWR_FV1 | RT5640_PWR_FV2);
#ifdef USE_ASRC
			snd_soc_write(codec, RT5640_GEN_CTRL1, 0x3f71); // joe_cheng : update to 3771 for power consummation
#else
...
//	INIT_WORK(&mclk_check_work, mclk_check_work_handler);
	...
	INIT_DELAYED_WORK(&spk_amp_work, do_spk_amp);
