...
#include "uG31xx_API.h"
  ...
  #define MEASUREMENT_VERSION      (_T("Measurement $Rev: 15 $"))

#else   ///< else of uG31xx_OS_WINDOWS

  #define MEASUREMENT_VERSION      ("Measurement $Rev: 15 $")
...
void ConvertExtTemperature(MeasDataInternalType *obj)
{
  ...
  UG31_LOGI("[%s]: NTC resistance = %d x %d / %d = %d\n", __func__,
            obj->info->codeExtTemperature, obj->info->sysData->ggbParameter->R, tmp16, tmp32);
...
void CalculateAdc1ConvertTime(MeasDataInternalType *obj)
{
     ...
     (tmp32 < (MINIMUM_ADC1_CONVERSION_TIME*TIME_CONVERT_TIME_TO_MSEC)))
  {
    ...
    UG31_LOGE("[%s]: Delta ADC Count      = %d\n", __func__, obj->codeCounter - obj->info->lastCounter);
    UG31_LOGE("[%s]: Old ADC Convert Time = %d\n", __func__, obj->info->adc1ConvertTime);
    UG31_LOGE("[%s]: New ADC Convert Time = %d\n", __func__, tmp32);
    UG31_LOGE("[%s]: ***************************************************************************************\n", __func__);
    UG31_LOGE("[%s]: ***************************************************************************************\n", __func__);
  ...
  UG31_LOGI("[%s]: Conversion Time = %d ((%d - %d)/%d)\n", __func__, 
            obj->info->adc1ConvertTime, obj->codeCounter, obj->info->lastCounter, obj->info->deltaTime);
  obj->info->lastCounter = obj->codeCounter;
}
...
void ConvertCharge(MeasDataInternalType *obj)
{
  ...
  UG31_LOGN("[%s]: Board Factor (%d/%d) -> %d\n", __func__, 
            ...
            BoardFactor[GET_PRODUCT_TYPE(obj->info->sysData->ggbParameter->NacLmdAdjustCfg)].current_offset, tmp32);
...
void CountCycleCount(MeasDataInternalType *obj)
{
  ...
  {
    UG31_LOGE("[%s]: No cycle count calculation because no discharging capacity measured.\n", __func__);
    return;
  }
  ...
  if(obj->info->cycleCountBuf >= obj->info->sysData->ggbParameter->CycleCountThrd)
  {
    ...
    UG31_LOGI("[%s]: Update cycle count = %d (%d >= %d)\n", __func__,
              obj->info->cycleCount, obj->info->cycleCountBuf, obj->info->sysData->ggbParameter->CycleCountThrd);
  }
}
