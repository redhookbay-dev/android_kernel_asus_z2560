...
void CreateBackupBuffer(BackupDataType *data)
{
  ...
  data->backupBuffer = (_backup_u8_ *)upi_malloc((_upi_u32_)data->backupBufferSize);
...
void PrepareData(BackupDataType *data)
{
  ...
  UG31_LOGN("[%s]: Driver version = %d (%d)\n", __func__, driverVer, UG31XX_DRIVER_VERSION);
...
_backup_u8_ CheckBackupFile(BackupDataType *data)
{
          ...
          (upi_memcmp(orgSysData, data->sysData, sizeof(SystemDataType)) != 0))
  {
    ...
    return (CHECK_BACKUP_FILE_STS_NEED_UPDATE);
  }
  else
  {
...
void UpiInitBackupData(BackupDataType *data)
{
  ...
  while(data->backupSuspendIdx)
  {
    ...
    if(data->backupSuspendData[data->backupSuspendIdx] != _UPI_NULL_)
    {
      upi_free(data->backupSuspendData[data->backupSuspendIdx]);
    }
    data->backupSuspendData[data->backupSuspendIdx] = (BackupSuspendDataType *)upi_malloc(sizeof(BackupSuspendDataType));
    ...
    UG31_LOGN("[%s]: data->backupSuspendData[%d] = %d\n", __func__, 
              data->backupSuspendIdx, data->backupSuspendData[data->backupSuspendIdx]);
  }
}
