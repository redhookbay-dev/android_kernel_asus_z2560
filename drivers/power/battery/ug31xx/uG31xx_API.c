...
_upi_bool_ MPK_active = _UPI_FALSE_;
  ...
  #define UG31XX_API_VERSION      (_T("UG31XX API $Rev: 15 $"))

#else

  #define UG31XX_API_VERSION      ("UG31XX API $Rev: 15 $")
...
#endif
{
  ...
  *pObj = (char *)upi_malloc(sizeof(struct ug31xx_data));
  ...
  else
  {
    ...
    ChkResumeData(pUg31xx, &batInfoBefore, totalTime, deltaQC);
    pUg31xx->batteryInfo.NAC = (_upi_u16_)pUg31xx->capData.rm;
    pUg31xx->batteryInfo.LMD = (_upi_u16_)pUg31xx->capData.fcc;
    pUg31xx->batteryInfo.RSOC = (_upi_u16_)pUg31xx->capData.rsoc;
    UpiResetCoulombCounter(&pUg31xx->measData);
...
#endif  ///< end of defined (uG31xx_OS_WINDOWS)
{
     ...
     (pUg31xx->backupData.icDataAvailable == BACKUP_BOOL_FALSE))
  {
    ...
    pUg31xx->backupData.icDataAvailable = BACKUP_BOOL_TRUE;
    UG31_LOGN("[%s]: Restore file routine START\n", __func__);
    ...
    if(rtn == _UPI_TRUE_)
    {
      ...
      tmp8 = (_upi_s8_)pUg31xx->capData.rsoc - (_upi_s8_)pUg31xx->sysData.rsocFromIC; 
      if((tmp8 < BACKUPFILE_RELOAD_RSOC_THRES) &&
         (tmp8 > -1*BACKUPFILE_RELOAD_RSOC_THRES))
      {
        UpiSaveBatInfoTOIC(&pUg31xx->sysData);
        pUg31xx->capData.rm = (_cap_u16_)pUg31xx->sysData.rmFromIC;
        pUg31xx->capData.fcc = (_cap_u16_)pUg31xx->sysData.fccFromIC;
    ...
    UG31_LOGN("[%s]: Restore file routine END\n", __func__);
  }
...
#define MAXIMUM_CURRENT_RANGE     (20)
 ...
 * @return  GGSTATUS
 */
void upiGG_GetBoardOffset(char *pObj)
{
  struct ug31xx_data *pUg31xx;
  _upi_s16_ upperBound;
  _upi_s16_ lowerBound;
  _upi_s16_ rawCurr;

  pUg31xx = (struct ug31xx_data *)pObj;

  upperBound = MAXIMUM_CURRENT_RANGE;
  lowerBound = upperBound*(-1);
  rawCurr = (_upi_s16_)pUg31xx->measData.curr;
  rawCurr = rawCurr + pUg31xx->measData.ccOffsetAdj;

  if((pUg31xx->measData.curr < upperBound) && (pUg31xx->measData.curr > lowerBound))
  {
  ...
  UG31_LOGI("[%s]: Board offset = %d (%d)\n", __func__, pUg31xx->measData.ccOffsetAdj, rawCurr);
}
...
void upiGG_SetCapacitySuspendMode(char *pObj, _upi_bool_ inSuspend)
{
  ...
  /// [AT-PM] : Set for capacity module ; 12/10/2013
  if(!(pUg31xx->backupData.backupNacLmdAdjustCfg & NAC_LMD_ADJUST_CFG_PARSER_FORCE_RSOC_STEP_1_DISABLE))
  {
    if(inSuspend == _UPI_TRUE_)
    {
      pUg31xx->sysData.ggbParameter->NacLmdAdjustCfg = pUg31xx->sysData.ggbParameter->NacLmdAdjustCfg | NAC_LMD_ADJUST_CFG_PARSER_FORCE_RSOC_STEP_1_DISABLE;
    }
    else
    {
      pUg31xx->sysData.ggbParameter->NacLmdAdjustCfg = pUg31xx->sysData.ggbParameter->NacLmdAdjustCfg & (~NAC_LMD_ADJUST_CFG_PARSER_FORCE_RSOC_STEP_1_DISABLE);
...
#define LKM_AVG_TEMPERATURE_COUNT         (16)
...
static unsigned char *lkm_shell_ap_name = _UPI_NULL_;
 ...
 * @return  GGSTATUS
 */
GGSTATUS lkm_update_procedure(void)
{
  GGSTATUS rtn;
  GG_DEVICE_INFO *devInfo;
...
#define UPI_POLLING_TIME_NEAR_OT_THRD   (500)
#define UPI_POLLING_TIME_NEAR_UT_THRD   (50)
#define UPI_POLLING_TIME_OT_THRD        (550)
...
_upi_s32_ polling_time_algorithm(void)
{
  ...
  else
  {
    ...
    else if(ug31xx->batteryInfo.RSOC == CONST_PERCENTAGE)
    {
      polling_time = 60;
    }
    else
...
int lkm_set_charger_full(char is_full)
{
  ...
  /// [AT-PM] : Reset coulomb counter ; 07/18/2013
  ...
  if(is_full == UG31XX_TAPER_REACHED)
  {
    ...
    ug31xx->capData.rm = (_cap_u16_)ug31xx->batteryInfo.NAC;
    ug31xx->capData.fcc = (_cap_u16_)ug31xx->batteryInfo.LMD;
    ug31xx->capData.rsoc = (_cap_u8_)ug31xx->batteryInfo.RSOC;
  }
...
int lkm_get_full_charge_status(void)
{
  ...
  ug31xx = (struct ug31xx_data *)lkm_gauge;
  fc_sts = ((ug31xx->capData.fcSts == CAP_TRUE) ? _UPI_TRUE_ : _UPI_FALSE_);

  return ((fc_sts == CAP_TRUE) ? 1 : 0);
}
 ...
 * @return  0 if success
 */
int lkm_calibrate_offset(void)
{
  upiGG_GetBoardOffset(lkm_gauge);
  return (0);
}
