...
static void aicl_dete_worker(struct work_struct *dat);
...
static void aicl_cur_control(int usb_state)
{
    ...
    if (usb_state != AC_IN) {
        dev_info(&smb345_dev->client->dev,
            "%s: do nothing without AC plugged in.\n",
            __func__);
        return;
    }
...
int smb345_charging_toggle(bool on)
{
    int ret = 0;
...
static bool pad_pwr_supply()
{
    ...
    */
    ...
    if (ret)
        dev_info(&smb345_dev->client->dev, "%s: TRUE\n", __func__);
    else
        dev_info(&smb345_dev->client->dev, "%s: FALSE\n", __func__);
