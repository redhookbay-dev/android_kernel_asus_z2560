...
#if defined(CONFIG_ME302C_BATTERY) || \
    defined(CONFIG_ME372CG_BATTERY) || \
    ...
    if (cable_status == USB_ADAPTER || cable_status == USB_PC) {
        ...
        */
        if (percentage >= 60 && tmp_batt_info.eng_charging_limit) {
            if (tmp_batt_info.tbl_chgr &&
                ...
                tmp_batt_info.tbl_chgr->charging_toggle(false);
        ...
        */
        ...
        if (status == POWER_SUPPLY_STATUS_CHARGING)
            if (tmp_batt_info.tbl_chgr &&
                tmp_batt_info.tbl_chgr->charging_toggle)
                tmp_batt_info.tbl_chgr->charging_toggle(true);
        if (percentage == 100)
                ...
                union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_CHARGE_NOW:
        val->intval = bq27520_asus_battery_dev_nominal_available_capacity();
        break;
#endif
