...
#endif
#if defined(CONFIG_SN280X_CHARGER)
extern int sn280x_chgr_state(int usb_state);
#endif
#ifdef CONFIG_HID_ASUS_PAD_EC
extern int ite8566_usb_cable_status(int usb_state);
...
extern void asus_pad_battery_set_cable_status(int usb_state);
#endif
#if CONFIG_TOUCHSCREEN_HIMAX_HX8528
extern int hx8528_cable_status(int status);
#endif
...
#ifdef CONFIG_HID_ASUS_PAD_EC
static int dock_owner_notify(struct notifier_block *this,unsigned long owner, void *data);
#endif
#ifdef CONFIG_EEPROM_PADSTATION
...
inline void notify_cable_status(int type)
{
	#if CONFIG_HID_ASUS_PAD_EC
	ite8566_usb_cable_status(type);
	#endif
	#if defined(CONFIG_ME302C_BATTERY_SMB347) || defined(CONFIG_ME372CG_BATTERY_SMB345)
	pr_info("%s\n", __func__);
	setSMB345Charger(type);
	#endif
	#if defined(CONFIG_SN280X_CHARGER)
	sn280x_chgr_state(type);
	#endif
	#ifdef CONFIG_TX201LA_BATTERY_EC
...
#ifdef CONFIG_HID_ASUS_PAD_EC
static struct notifier_block early_dock_notifier = {
        .notifier_call =    dock_owner_notify,
};

static struct notifier_block late_dock_notifier = {
        .notifier_call =    dock_owner_notify,
};
...
bool hsot_mode;
static int dock_owner_notify(struct notifier_block *this,
                            unsigned long owner, void *data)
{
	struct penwell_otg		*pnw = the_transceiver;
	struct intel_mid_otg_xceiv	*iotg = &pnw->iotg;

	printk("USB : dock_owner_notify owner  = %d\n",owner);
       if (owner == SYSTEM_ANDROID){ 
		hsot_mode=true;
       }else{
		hsot_mode=false;
		iotg->hsm.a_vbus_vld = 0;
	}
	return NOTIFY_OK;
}
#endif
		...
		switch (event)
			{
	                case P01_USB_IN:
					hsm->b_sess_vld =1;
					#if defined(CONFIG_PF400CG) && defined(CONFIG_ME372CG_BATTERY_SMB345)
					pad_in_otg(ENABLE_5V);
					#endif
					break;
	                case P01_AC_IN:
					hsm->b_sess_vld =1;
...
static int penwell_otg_set_vbus(struct usb_otg *otg, bool enabled)
{
	...
	if (is_clovertrail(to_pci_dev(pnw->dev))) {
		...
		if ((!enabled) && (pnw->iotg.hsm.id == ID_ACA_A
			...
			#endif
			#if defined(CONFIG_SN280X_CHARGER)
			sn280x_chgr_state(enabled? ENABLE_5V : DISABLE_5V);
			#endif
			#if CONFIG_HID_ASUS_PAD_EC
...
int check_cable_status(void)
{
	...
	if (pnw->psc_cap.chrg_type != NULL){
		spin_lock_irqsave(&pnw->charger_lock, flags);
		charger_type = pnw->psc_cap.chrg_type;
		spin_unlock_irqrestore(&pnw->charger_lock, flags);
		if (charger_type == POWER_SUPPLY_CHARGER_TYPE_USB_SDP)
			return USB_IN;
		if (charger_type == POWER_SUPPLY_CHARGER_TYPE_USB_DCP ||charger_type == POWER_SUPPLY_CHARGER_TYPE_SE1)
			return AC_IN;
		if (charger_type == POWER_SUPPLY_CHARGER_TYPE_NONE)
			return CABLE_OUT;
	}
	else
		return CABLE_OUT;
...
static void penwell_otg_eye_diagram_optimize(void)
{
	...
	if (is_clovertrail(to_pci_dev(pnw->dev))) {
		...
		if (Read_PROJ_ID()==PROJ_ID_ME372CG)
			value = 0x7a;
		else if (Read_PROJ_ID()==PROJ_ID_ME175CG)
			value = 0x77;
		else
			value = 0x7f;
	} else {
		...
		const struct pci_device_id *id)
{
	...
	//20130326 Eric add for ID wakeup ++
...
#ifdef CONFIG_HID_ASUS_PAD_EC
        unregister_dock_atow_early_notifier(&early_dock_notifier);
        unregister_dock_wtoa_late_notifier(&late_dock_notifier);
