       ...
       if(status > 0 && reg_id > 0){
            ...
            if(is_Mode_APROM()){
                    ...
                    do {
                        ...
                        //op_state = 1; // Walf FW not support
                        retries--;
    ...
    if(g_prop_virtualRemoveEnabled){
        if(g_i2c_bus_suspended)
            printk("%s: Bus Suspended: Skip\r\n", __FUNCTION__);
                 ...
                 (st_PRE_CONNECTED==g_b_isP01Connected && !strncmp(devname, "LVDS", strlen(devname))) ) ){
            if(pad_exist()){
...
static void initP01(struct work_struct *work){
    ...
    if(st_CONNECTED==g_b_isP01Connected){
        ...
        uP_i2c_read_reg(MICROP_USB_DET, &usb_type);
        printk("skip first intr status = %x, input = %x, usb_det=%x\r\nCheck Devices Status on Pad\r\n",reg_intr_sta,reg_input, usb_type);
...
*/                        
		...
		case ASUS_MICROP_GET_PADPHONE_HW_ID:
			/*if(isPhoneA68()){
				ret=__put_user(g_ASUS_hwID, (int __user *)arg);
			}
			else if(isPhoneA80()){
				int p05_hwid = AX_MicroP_getHWID();
				ret=__put_user(p05_hwid, (int __user *)arg);
			}*/
			//+++ Eric
	...
	pm_message_t mesg)
{
    ...
    if(AX_MicroP_IsP01Connected() && pad_exist()){
        AX_MicroP_enablePinInterrupt(INTR_EN_VOL_DOWN | INTR_EN_VOL_UP | INTR_EN_PROXM_INT, 0);
...
static int microp_resume(struct i2c_client *client)
{
    ...
    if(AX_MicroP_IsP01Connected() && pad_exist()){
        ...
        AX_MicroP_enablePinInterrupt(INTR_EN_VOL_DOWN | INTR_EN_VOL_UP | INTR_EN_PROXM_INT, 1);
