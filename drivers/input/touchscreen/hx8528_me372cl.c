...
#define DRIVER_VERSION	"0.0.3"		//Josh modify 
...
static int himax_ts_poweron(void)
{
	...
	if (IC_TYPE == HX_85XX_C_SERIES_PWON)
	{
		...
		if(ret < 0) 
		{
			printk(KERN_ERR "[Himax] %s: HX_CMD_SETMICROOFF failed addr = 0x%x\n",__func__, himax_data->client->addr);
			goto send_i2c_msg_fail;
		}
		udelay(100);   
		...
		if(ret < 0) 
		{
			printk(KERN_ERR "[Himax] %s: HX_CMD_E3 failed addr = 0x%x\n",__func__, himax_data->client->addr);
			goto send_i2c_msg_fail;
		} 
		udelay(100);
	...
	else if (IC_TYPE == HX_85XX_A_SERIES_PWON)
	{
		...
		if(ret < 0) 
		{
			printk(KERN_ERR "[Himax] %s: HX_CMD_76 failed addr = 0x%x\n",__func__, himax_data->client->addr);
			goto send_i2c_msg_fail;
		} 
		udelay(100);
...
static ssize_t himax_get_fw_version(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%s-%s\n", himax_data->himax_firmware_version,DRIVER_VERSION);
}
static ssize_t himax_debug_log(struct device *dev, struct device_attribute *attr, char *buf, size_t count)
{	
	if (buf[0] == '0'){
...
static ssize_t himax_chip_raw_data_store(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(diag_command == 1)
	{
		...
		if(IS_ERR(filp)) 
		{
			printk(KERN_ERR "[Himax] %s: open /data/log/touch/ME372CL_touch_dc.txt failed\n", __func__);
			return 0;
		}
		oldfs = get_fs();
		set_fs(get_ds());
	...
	else if(diag_command == 2)
	{
		...
		if(IS_ERR(filp)) 
		{
			printk(KERN_ERR "[Himax] %s: open /data/log/touch/ME372CL_touch_diff.txt failed\n", __func__);
			return 0;
		}
		oldfs = get_fs();
		set_fs(get_ds());
	...
	else if(diag_command == 3)
	{
		...
		if(IS_ERR(filp)) 
		{
			printk(KERN_ERR "[Himax] %s: open /data/log/touch/ME372CL_touch_bank.txt failed\n", __func__);
			return 0;
		}
		oldfs = get_fs();
		set_fs(get_ds());
...
static int himax_firmware_check(void)
{
    ...
    int upgrade_version_checksum = 0;
    int i = 0, open_fail = 1;
	...
	sprintf(upgrade_tp_version, "%2.2X%2.2X \n",upgrade_fw[0x0085],upgrade_fw[0x0086]);
	printk("[Himax] %s: upgrade_tp_version: %02X, %02X, %02X, %02X.\n", __func__, upgrade_tp_version[0], upgrade_tp_version[1], upgrade_tp_version[2], upgrade_tp_version[3]);
	upgrade_version_checksum = ((upgrade_tp_version[0] << 24) | (upgrade_tp_version[1] << 16) | (upgrade_tp_version[2] << 8) | upgrade_tp_version[3]);
	
	printk("[Himax]: upgrade firmware verison = %s.\n",upgrade_tp_version);
	printk("[Himax]: upgrade firmware verison checksum = %d.\n",upgrade_version_checksum);
		
	if(upgrade_version_checksum > himax_data->firmware_version_checksum )
	{
		return 0;
	}
	else
...
static int himax_firmware_upgrade(int path){
	...
	if(result > 0)
	{
		...
		{	
			...
			if(fts_ctpm_fw_upgrade_with_sys_fs(upgrade_fw, result) == 0)
			{
				printk(KERN_INFO "[Himax] %s: ME372CL Himax hx8528-D48 chip upgrade error.\n", __func__);
				fw_update_complete = false;
			...
			{
				printk(KERN_INFO "[Himax] %s: ME372CL Himax hx8528-D48 chip upgrade success. \n", __func__);
				fw_update_complete = true;
		...
		{
			printk(KERN_INFO "[Himax] %s: ME372CL Current touch firmware is the latest version.\n", __func__);
		}
	}
	else
	{
		printk("[Himax] %s: ME372CL Himax hx8528-D48 chip not upgrade. \n", __func__);
	}
...
//	struct himax_ts_data *ts = i2c_get_clientdata(client);
	...
	{
		err = request_irq(himax_data->irq, himax_ts_irq_handler, IRQF_TRIGGER_FALLING, himax_data->client->name, himax_data->client);
	}
...
//#endif
	...
	if (err < 0) {
		printk( "[Himax] %s: could not register ME372CL touch misc device.\n",__func__);
	}
	//*********************************************************************************************************
