...
#define KXTJ9_CHIP_LOCATION_SR_ME175CG		3	//	X(0 , 1 , 0 );	Y(1 , 0 , 0);	Z(0 , 0 , -1)
#define KXTJ9_CHIP_LOCATION_SR_PF400CG		4	//	X(-1 , 0 , 0 );	Y(0 , 1 , 0);	Z(0 , 0 , -1)
#define KXTJ9_CHIP_LOCATION_SR_A400CG		5	//	X(-1 , 0 , 0 );	Y(0 , 1 , 0);	Z(0 , 0 , -1)
int g_ilocation=0;
...
static const struct {
	...
	{ 70,			ODR25F,		KXTJ9_RES_8BIT},				// 70,	31~70	UI MODE
	{ 250,			ODR12_5F,	KXTJ9_RES_8BIT},				// 160,	71~250	NORMAL MODE
	{ 0xFFFFFFFF,	ODR12_5F,	KXTJ9_RES_8BIT},				// 160,	251~max	NO POLL
...
//			}
		...
		case KXTJ9_CHIP_LOCATION_SR_PF400CG : 
		case KXTJ9_CHIP_LOCATION_SR_A400CG : 
			if(g_KXTJ9_CLIBRATION_6_PLANES){
				if( (tj9->accel_cal_sensitivity[0]==0)||(tj9->accel_cal_sensitivity[1]==0)||(tj9->accel_cal_sensitivity[2]==0) ){
					reportdata_x = rawdata_x*(-1);
					reportdata_y = rawdata_y;
					reportdata_z = rawdata_z*(-1);
					if(KXTJ9_DEBUG_MESSAGE) printk("alp : KXTJ9_CHIP_LOCATION_SR_PF400CG default\n");
				}
				else{
					reportdata_x = (-1024)*(rawdata_x - tj9->accel_cal_offset[0])/tj9->accel_cal_sensitivity[0];
					reportdata_y = 1024*(rawdata_y - tj9->accel_cal_offset[1])/tj9->accel_cal_sensitivity[1];
					reportdata_z = (-1024)*(rawdata_z - tj9->accel_cal_offset[2])/tj9->accel_cal_sensitivity[2];
					if(KXTJ9_DEBUG_MESSAGE) printk("alp : KXTJ9_CHIP_LOCATION_SR_PF400CG calibration\n");
				}
			}else{
				reportdata_x = (rawdata_x - tj9->accel_cal_offset[0])*(-1);
				reportdata_y = rawdata_y - tj9->accel_cal_offset[1];
		...
		case KXTJ9_CHIP_LOCATION_SR_ME175CG :
//			if(g_KXTJ9_CLIBRATION_6_PLANES){
				if( (tj9->accel_cal_sensitivity[0]==0)||(tj9->accel_cal_sensitivity[1]==0)||(tj9->accel_cal_sensitivity[2]==0) ){
...
//	for transfor form file to int --
	...
	if(g_KXTJ9_CLIBRATION_6_PLANES){
		...
		if( (g_ilocation == KXTJ9_CHIP_LOCATION_SR_PF400CG) || (g_ilocation == KXTJ9_CHIP_LOCATION_SR_A400CG) ){
			//for X axis
...
#ifdef CONFIG_ME175CG
	g_ilocation = KXTJ9_CHIP_LOCATION_SR_ME175CG;
#endif

#ifdef CONFIG_PF400CG
	g_ilocation = KXTJ9_CHIP_LOCATION_SR_PF400CG;
#endif

#ifdef CONFIG_A400CG
	g_ilocation = KXTJ9_CHIP_LOCATION_SR_A400CG;
#endif
