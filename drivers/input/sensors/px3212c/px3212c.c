...
static int ps_initial(struct px3212c_i2c_data *px3212c_data)
{
	int ret=0, PS_Configuration=0, PS_LEDControl=0, PS_MeanTime=0;

	// SW reset
	ret = i2c_smbus_write_byte_data(px3212c_i2c_client, SYSTEM_CONFIG, 0x04);
	if(ret< 0)  {
		if(PX3212C_STATUSMSG) printk( "[%s]	Write PS_Reset Failed (%d)\n",__FUNCTION__,ret);
		return ret;
	}
	if(PS_DEBUGMSG) printk( "[px3212c_%s]	PS_Reset \n",__FUNCTION__);
//add by leo for PS Mean Time setting ++

	ret = i2c_smbus_write_byte_data(px3212c_i2c_client, PS_MEAN_TIME, 0x00);// 000000 00: mean time = 12.5ms
	if(ret< 0)  {
		if(PX3212C_STATUSMSG) printk( "[%s]	Write PS_MEAN_TIME Failed (%d)\n",__FUNCTION__,ret);
		return ret;
	}
...
//add by leo for PS Mean Time setting --
	...
	ret = i2c_smbus_write_byte_data(px3212c_i2c_client, PS_LED_CONTROL, 0x33);//PS LED control: 0011 0011, LED pulse:11 (3 pulse), LED driver ratio:11 (100%)
	if(ret< 0)  {
	...
	ret = i2c_smbus_write_byte_data(px3212c_i2c_client, PS_CONFIG, 0xe4);//RS integrated time select:1110 (15T), PS gain:10, PS persist(interrupt filter):00 
	if(ret< 0)  {
...
static int px3212c_EC_event_notify(struct notifier_block *this, unsigned long event, void *ptr)
{
	...
	switch (event){
			...
			case P01_ADD: 
				...
				if(ret < 0)if(unlikely(debug)) printk( "[%s]		PX3212C Set Calibration value Failed \n",__FUNCTION__);
				else if(unlikely(debug)) printk( "[%s]		PX3212C Set Calibration value OK \n",__FUNCTION__);
				// Proximity sensor is Disable from HAL
				if(IsEnableSensor==0){	
					IsLinkedPad=1;
					// if p-sensor is open , close it.
					ret = i2c_smbus_read_byte_data(px3212c_i2c_client, SYSTEM_CONFIG);
					if(ret == 0x02){ 
						ret = ps_enable(px3212c_data, POWER_OFF);	
						if(ret < 0)  {
