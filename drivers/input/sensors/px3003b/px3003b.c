...
#define Default_PS_ThresholdValue 5 
...
static int px3003b_get_crosstalk_value(void); // Not Ready
static int px3003b_crosstalk(void); // Not Ready
#ifdef CONFIG_PF400CG
...
static int px3003b_crosstalk_proximity(void)
{
	...
	if(PS_AlreadyCrossTalk==FALSE){
		...
		if(ret<0){
			...
			err = px3003b_set_threshold(PS_ThresholdValue+PS_CrossTalk);
			if(err < 0)  {
